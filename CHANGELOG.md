...
### DesignGate 保真稿软豁免（默认要，小需求可显式豁免）

- 默认所有前端（尤其 App / Web 页面）需保真稿；小需求确实不需要时，可在 `exceptions.yml` 声明一条 `gate_override(gate: DesignGate, reason: ...)` 显式豁免——留痕、可审计（写入 gate evidence），区别于"不建 design/ 目录静默绕过"。`reason` 必填，`approved_by` 选填（不填也放行但标记为自我声明）。
- 堵暗门：无 `design/` 目录时不再直接判"无 UI"放行；先对 spec/request 推断 UI 类型，判为 C/B 端且无豁免则 BLOCK 并指向两条正门（补稿 / 写豁免），判不出或无 UI 信号才放行（不误伤纯后端改动）。
- 新增 dev selftest `designgate_waiver_contract`（5 条正反例）；dev selftest 41/41。

## [5.19.0] - 2026-07-16

### init 产出干净目录 + 仅治理层可选

- **物理拆分 skill/dev**：DeliverHQ 自身开发资产（examples/evals/superpowers、dev 文档、dev-CR、测试套件 selftest/selftest_contracts/eval_routing/gate_contract_check、测试夹具、归档脚本）移至仓库内 `dev/`，不随包发布、不进 `init-project` 产物。`copyDir` 拷到的 `skill/` 是干净的运行时。
- **组织记忆去 dogfood**：`docs/MEMORY.md`、`attention.md`、`STATE.md` 改为项目模板，`.claude/memory` 置空——新项目不再混入框架自身的 CR/决策/记忆。
- **doctor 用轻量 `health_check.py`**（骨架/dir-graph/脚本可编译），全量 40 项 selftest 下沉 `dev/`；`selftest` 命令别名到 `health_check`。
- **新增 `governance-only` profile + init 交互选择**：默认"仅治理层"（只建 `DeliverHQ/`，不碰业务目录结构，适合库/CLI/移动端/已有结构的项目）；`--governance-only`/`--profile`/`--yes` 可非交互直选；`fullstack-web` 仍可选。
- **修复发布态 skeleton bug**：`MIGRATION.md`/`ROLLBACK.md`/`docs/reports/*` 此前被 npm 排除但 check_skeleton 必需，导致真实安装后 doctor 失败；现随包发布。清理冗余 `.npmignore`/`files` 排除项（改为目录隔离，单一事实源）。

### 验证

- 两种 profile init 产物各 ~150 文件（原 491），零业务污染、零 dev 资产、`.claude/memory` 为 `{}`。
- dev selftest 40/40；两种 profile 的 check_skeleton/structuregate/doctor 全过；npm pack 零 dev/ 泄漏。

## [5.18.0] - 2026-07-16

### 统一入口、可靠性与记忆生命周期

- 新增只读 `deliverhq go` 入口，结合项目内活跃 CR、下一阶段与工件预检，输出无占位符的具体推进命令；歧义和人工审批场景 fail-closed。
- 扩展 MemoryStore 的语义指纹、重复计数、证据与 `active/superseded/deprecated` 生命周期，并兼容旧索引。
- 新增 `deliverhq bootstrap` 只读扫描既有项目、生成 `*.candidate` 上下文；新增 knowledge lifecycle 检查、lazy CR flow 控制、AgentGate 治理检查。
- `--help` 输出首行显示版本号（读 `package.json`，与 `--version` 同源），便于确认所装版本。
- 修复 Windows 控制台编码、语义 CR-ID、默认分支探测和 pytest/npm 发布卫生闭环。

> 说明：5.4.0–5.17.0 的历史版本此前未打 git tag；自 5.18.0 起，每次版本号变更配套打 `vX.Y.Z` tag，使 `github:kevinwanghd/deliverhq#vX.Y.Z` 可安装指定版本，`npm install -g github:kevinwanghd/deliverhq` 始终取 main HEAD（最新）。
