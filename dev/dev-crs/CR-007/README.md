...
2. 替换所有模板变量（`CR-007`、`统一入口、验证闭环与记忆生命周期基础` 等）
3. 填充当前日期和提出人
...
# 手动替换变量（容易遗漏）
find . -type f -exec sed -i 's/CR-007/CR-001/g' {} \;
find . -type f -exec sed -i 's/统一入口、验证闭环与记忆生命周期基础/实现用户登录功能/g' {} \;
find . -type f -exec sed -i 's/2026-07-13/2026-06-11/g' {} \;
```
...
|---|---|---|
| `CR-007` | CR 编号 | CR-001 |
| `统一入口、验证闭环与记忆生命周期基础` | CR 名称 | 实现用户登录功能 |
| `2026-07-13` | 日期 | 2026-06-11 |
| `用户` | 提出人 | 产品经理 |
