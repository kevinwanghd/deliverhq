...
2. 替换所有模板变量（`CR-001`、`升级项目使用 DeliverHQ v4.6 框架` 等）
3. 填充当前日期和提出人
...
# 手动替换变量（容易遗漏）
find . -type f -exec sed -i 's/CR-001/CR-001/g' {} \;
find . -type f -exec sed -i 's/升级项目使用 DeliverHQ v4.6 框架/实现用户登录功能/g' {} \;
...
|---|---|---|
| `CR-001` | CR 编号 | CR-001 |
| `升级项目使用 DeliverHQ v4.6 框架` | CR 名称 | 实现用户登录功能 |
