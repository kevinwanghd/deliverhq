...
2. 替换所有模板变量（`CR-003`、`实现 Loop Mode 执行引擎` 等）
3. 填充当前日期和提出人
...
# 手动替换变量（容易遗漏）
find . -type f -exec sed -i 's/CR-003/CR-001/g' {} \;
find . -type f -exec sed -i 's/实现 Loop Mode 执行引擎/实现用户登录功能/g' {} \;
...
|---|---|---|
| `CR-003` | CR 编号 | CR-001 |
| `实现 Loop Mode 执行引擎` | CR 名称 | 实现用户登录功能 |
