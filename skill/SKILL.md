...
- DevPhase Handoff（交接，不自动写码）、Gate Contract、health_check 自检
...
- **模式 1 初始化**：复制 `DeliverHQ/` 到项目根 → `check_skeleton.py DeliverHQ` → 填 `docs/CONTEXT.md` → 调 `dir-graph.yaml` → `health_check.py`（或 `npx deliverhq doctor`）。
...
- [ ] 运行 `python scripts/health_check.py` 全部通过
...
一键自检：`python scripts/health_check.py`（骨架完整 / dir-graph 合法 / 脚本可编译）。

---

**版本**：v5.19.0 ｜ **一句话**：文档门禁 + 证据驱动 + 对抗式验证（信证据不信声明）；日常 CR 用 5 个动词收口。
