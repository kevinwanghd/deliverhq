...
#
# 注意：estimate_cost, print_cost_estimate, has_gate_cache 已从 orchestrator_routing 导入。
#       analyze_cr_size, decompose_cr, route_situation 同理。
#       如需修改这些函数，请修改 deliverhq/routing.py（单一事实源）。
...
class SkillOrchestrator:
    ...
    def execute_skill(self, skill_type: str, cr_path: str, **kwargs) -> bool:
        ...
        if skill_type in VERB_NO_ARG_STEPS:
            ...
            script_args = [args_value]

        result = run_script(script_path, script_args, env=SUBPROCESS_ENV, timeout=600)
        if result.timed_out:
    ...
    def _run_retry_status(self, cr_path: str):
        ...
        if not script.exists():
            return
        try:
            r = run_script(script, [cr_path, "status"], timeout=20)
            if r.stdout:
...
_has_gate_cache = has_gate_cache
