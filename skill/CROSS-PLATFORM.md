...
> **已验证跨平台运行**：核心框架在 Windows、macOS、Linux 上均可使用。
...
```bash
# 1. 解压包
tar xzf deliverhq-*.tar.gz   # 或 7z x deliverhq-*.7z
cd DeliverHQ
...
# 3. 门禁契约（测试 Gate 正反例）
# （框架开发者可另跑全量自测：python dev/scripts/selftest.py）
```

如果 health_check 3/3 通过，说明框架在该平台可用。

## 六、常见问题

**Q: Windows 上 `python` 命令找不到？**  
A: 安装时勾选"Add Python to PATH"，或用 `py` 命令（Python Launcher）：`py scripts/health_check.py`
...
**总结**：DeliverHQ 框架本身已跨平台适配。用户只需确保 Python + Git 在 PATH，并让自定义验证命令遵循跨平台规范。
